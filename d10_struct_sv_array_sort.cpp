...
void fn_Students(){
    ...
    //sap xep ds sv theo thu tu ho ten
    SINHVIEN temp; //khai bao bien temp kieu cau truc STUDENT
    for (int i = 0; i < n-1; i++)
    {
        for (int k = i+1; k < n; k++)
        {
            if(strcmp(ds[i].name, ds[k].name)>0){
                //hoan doi 2 doi tuong sv o dong i va k
                temp = ds[i];
                ds[i] = ds[k];
                ds[k] = temp;
            }
     ...
     //vong lap in ra danh sach sinh vien - sau khi sap xep theo cot ho ten
    printf("\n\n Danh sach sinh vien - theo thu tu ho ten \n");
    for (int i = 0; i < n; i++)
    {
        printf(" %4s %-20s %02d-%02d-%4d %3d \n", ds[i].id, ds[i].name, 
        ds[i].birthday.dd, ds[i].birthday.mm, ds[i].birthday.yy,
    ...
    //sap xep ds sv theo thu tu diem thi - cao den thap
    for (int i = 0; i < n-1; i++)
    {
        for (int k = i+1; k < n; k++)
        {
    ...
    //vong lap in ra danh sach sinh vien - sau khi sap xep theo cot diem
    printf("\n\n Danh sach sinh vien - theo thu tu diem ket qua \n");
    for (int i = 0; i < n; i++)
    {
        printf(" %4s %-20s %02d-%02d-%4d %3d \n", ds[i].id, ds[i].name, 
        ds[i].birthday.dd, ds[i].birthday.mm, ds[i].birthday.yy,
